...
def process_video_qa(video_path, api_key, question, options, example):
...
{example}
    ...
    prompt = prompt_template.format(
        systerm=systerm, question=question, options_text=options_text, example=example
    )
    # print(prompt)
...
def process_item(
    ...
    try:
        # 处理视频问答
        result = process_video_qa(video_path, api_key, question, options, example)
        # 保存结果到文件
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        default="./results/1stage/gemini_v2",
        help="结果保存的文件路径",
    )
    parser.add_argument(
        "--max_process",
